...
# **DETECTION JSDM model** with *5 seasons* 

Fall 2018, Spring 19, Fall 2019, Spring 20 (covid), Fall 20 (covid)

Run JSDM model of carnivore detection data from citizen scientists camera traps 
...
* **Response variable**: 
  - *Presence/Absence carnivore*: detection of the species (at least once during the sampling month) 
    - Red Fox
    - Raccoon
...
* **Explanatory variables**:
  ...
  - *(Domestic) Cat*: categorical (presence/absence)
...
## Load and clean data
```{r}
# load species detections
species_file <- list.files(data_wd, pattern = c("Detection.+ordered"))
species_file

det_species <- readRDS(file.path(data_wd, species_file[2]))
head(det_species)
str(det_species)

# user_uid 1769 and 1869 are season 5
## Corrected data

det_species$project_phase[det_species$User_uid == 1769 | det_species$User_uid == 1869] <- 5
...
# Join both data sets by user id and season
det_envir <- left_join(det_species, envcov, by = c("User_uid", "project_phase")) %>% 
...
det_envir[is.na(det_envir$noise_100),]
det_envir[is.na(det_envir),]

det_envir %>% 
  group_by(project_phase) %>% 
  summarise(n_cam = n_distinct(User_uid))
# write.csv(det_envir, paste0(data_wd, "/Detection_envir_alldata_nodupl_noNa_mar21.csv"), row.names = FALSE)
# det_envir <- read.csv(paste0(data_wd, "/Detection_envir_alldata_nodupl_noNa_mar21.csv")) 
```
  project_phase n_cam
          <dbl> <int>
1             1   150
...
## Prepare data for DETECTION JSDM model
```{r}
head(det_envir)
# response
species_matrix <- det_envir %>%
  dplyr::select(Red_fox, Raccoon, Marten) %>%
  as.matrix()
  
head(species_matrix)

# covariates: select env variables 
covariates <- det_envir %>%
...
site.id <- as.factor(det_envir$User_uid)
...
head(covariates)
XFormula_det = ~ garden_size + Local_tree_cover + fence_height + compost +
  pop_100 + imperv_100 + noise_100 + tree_cover_100 + distance_border +
  season + covid + Cat


# BINOMIAL DISTRIBUTION: Untransformed data (presence/absence)
## Model scales the data prior to running

# Fit the model
m <- Hmsc(Y = species_matrix, XData = covariates, XFormula = XFormula_det,
          studyDesign=studyDesign, ranLevels=list(site=rL),
          # TrData = NA, TrFormula = NA,
          distr = "probit") # for 2 values: presence/absence
...
filename = paste0(det_results_wd, "/CT_carnivores_DET_5seasons_mar21.rds")
...
for (i in 1:length(convergence.names)){
  write.csv(convergence.list[[i]], paste0(det_results_wd,
                                          "/", convergence.names[i], ".csv")) 
          # row.names = FALSE)
}

# Graphical output
png(paste0(det_results_wd, "/CTcar_DET_5seasons_model_convergence_hist.png"), width = 800, height = 1000,
    pointsize = 20)
par(mfrow=c(2,2))
hist(ess.beta, main = "ess(beta)_CTcar_DET_5seasons")
hist(ess.omega, main = "ess(omega)_CTcar_DET_5seasons")
hist(gd.beta, main = "psrf(beta)_CTcar_DET_5seasons")
hist(gd.omega, main = "psrf(omega)_CTcar_DET_5seasons")
dev.off()
...
MCMCtrace(mpost$Beta, 
          pdf = TRUE, 
          open_pdf = FALSE,
          filename = "CTcar_DET_5seasons_MCMCtrace_beta",
          wd = det_results_wd)
MCMCtrace(mpost$Omega[[1]], 
          pdf = TRUE, 
          open_pdf = FALSE,
          filename = "CTcar_DET_5seasons_MCMCtrace_omega",
          wd = det_results_wd)
...
# write.csv(autocorrBeta, paste0(det_results_wd,
#                                           "/autocorr_beta.csv"))

(autocorrOmega <- autocorr(mpost$Omega[[1]]))
# write.csv(autocorrOmega, paste0(det_results_wd,
#                                           "/autocorr_omega.csv"))

plot(mpost$Beta)
plot(mpost$Omega[[1]])

# saveRDS(mpost, paste0(det_results_wd, "/mpost_coda_DET_5seasons.rds"))
# mpost <- readRDS(paste0(det_results_wd, "/mpost_coda_DET_5seasons.rds"))
``` 
...
# saveRDS(preds, paste0(det_results_wd, "/preds_DET_5seasons.rds"))

preds.values <- apply(abind(preds,along=3),c(1,2), median) # Median of the predictions
write.csv(preds.values, paste0(det_results_wd, 
                               "/Predicted_values_median_CTcar_DET_5seasons.csv"))
preds.values.mean <- apply(abind(preds, along = 3), c (1,2), mean) # Mean of the predictions
write.csv(preds.values.mean, paste0(det_results_wd, 
                                    "/Predicted_values_mean_CTcar_DET_5seasons.csv"))
...
r2.explanatory.sp
# write.csv(r2.explanatory.sp, paste0(det_results_wd, "/DET_5seasons_explanatory_power.csv"), 
#           row.names = FALSE)
```
$RMSE
[1] 0.4068908 0.2241228 0.2085726

$AUC
[1] 0.8236247 1.0000000 1.0000000

$TjurR2
[1] 0.1403570 0.5618277 0.5723140
...
ggplot(data = temp.r2, aes(y=r2, x=species))+
  geom_point()
write.csv(modelr2.explanatory, paste0(det_results_wd,                                       "/CTcar_DET_5seasons_explanatory_R2_default.csv"), row.names = FALSE)
...
ggplot(data = as.data.frame(modelr2.explanatory$TjurR2), aes(y=modelr2.explanatory$TjurR2, x=temp.r2$species))+
  ...
  ggtitle(paste0("Urban Mammals - Default Tjur R2 species \n R2 = ", 
                   round(mean(modelr2.explanatory$TjurR2, na.rm = TRUE), 2)))
ggsave(paste0(det_results_wd, "/Plot_R2_species_default_DET_5seasons.png"))
...
ggsave(paste0(det_results_wd, "/Plot_R2_species_manual_DET_5seasons.png"))
```
...
ggsave(paste0(det_results_wd, "/Plot_R2_sites_manual_DET_5seasons.png"))
...
ggplot(data = as.data.frame(R2.sp), aes(y=V1, x=colSums(m$Y) / m$ny))+
  ...
  ggtitle(paste0("Urban Mammals - Explanatory R2: proportion of sites present\n R2 = ", 
                   round(mean(R2.site, na.rm = TRUE), 2)))
ggsave(paste0(det_results_wd, "/Plot_R2_prevalence_manual_DET_5seasons.png"))


# compare predicted and observed col sums (one data point is a species)
png(paste0(det_results_wd, '/Plot_Obs_vs_Pred_sp_DET_5seasons.png'))
...
# saveRDS(preds_tmp, paste0(det_results_wd, "/preds_ppc_DET_5seasons.rds"))
preds_tmp <- readRDS(paste0(det_results_wd, "/preds_ppc_DET_5seasons.rds"))
...
### red fox
# Extract observed y
y_fox <- as.vector(species_matrix$Red_fox)


# extract 50 predicted y
test <- data.frame(Species = rep("Red_fox", nrow(preds_tmp[,,1])))
for(i in 1:dim(preds_tmp)[3]){
  x <- as.vector(preds_tmp[,,i][,"Red_fox"])
  test[,ncol(test)+1] <- x
  colnames(test)[ncol(test)] <- paste0("fox", i)
}
test
ypred_fox <- t(as.matrix(test[,-1])) # probabilities

# transform into 1/0
library(Rlab)
test_bern <- test

A <- function(x) rbern(1, x)
test_bern <- apply(test_bern[,-1],c(1,2), A) # bernoulli: 0/1
ypred_bern_fox <- t(test_bern)


# Run function - probabilities
(ppc_fox <- pp_check(y_fox, ypred_fox, ppc_dens_overlay) +
  ggtitle("PPC - Fox detection"))
ggsave(plot = ppc_fox, filename = paste0(plots_wd, "/ppc_DET_fox.png"), 
       dpi = 600, width = 9, height = 5.8)

# Run function - bernoulli
(ppc_fox <- pp_check(y_fox, ypred_bern_fox, ppc_dens_overlay) +
  ggtitle("PPC - Fox detection"))
ggsave(plot = ppc_fox, filename = paste0(plots_wd, "/ppc_DET_bern_fox.png"), 
       dpi = 600, width = 9, height = 5.8)


### raccoon
# Extract observed y
y_rac <- as.vector(species_matrix$Raccoon)
...
test_bern <- test
test_bern <- apply(test_bern[,-1],c(1,2), A) # bernoulli: 0/1
ypred_bern_raccoon <- t(test_bern)


# Run function
ppc_rac <- pp_check(y_rac, ypred_raccoon, ppc_dens_overlay) +
  ggtitle("PPC - Raccoon detection")
ggsave(plot = ppc_rac, filename = paste0(plots_wd, "/ppc_DET_raccoon.png"), 
       dpi = 600, width = 9, height = 5.8)

# Run function - bernoulli
(ppc_rac2 <- pp_check(y_rac, ypred_bern_raccoon, ppc_dens_overlay) +
  ggtitle("PPC - Raccoon detection"))
ggsave(plot = ppc_rac2, filename = paste0(plots_wd, "/ppc_DET_bern_raccoon.png"), 
       dpi = 600, width = 9, height = 5.8)


### marten
# Extract observed y
y_marten <- as.vector(species_matrix$Marten)
...
test_bern <- test
test_bern <- apply(test_bern[,-1],c(1,2), A) # bernoulli: 0/1
ypred_bern_marten <- t(test_bern)


# Run function
ppc_mart <- pp_check(y_marten, ypred_marten, ppc_dens_overlay) +
  ggtitle("PPC - Marten detection")
ggsave(plot = ppc_mart, filename = paste0(plots_wd, "/ppc_DET_marten.png"), 
       dpi = 600, width = 9, height = 5.8)

# Run function - bernoulli
(ppc_mart2 <- pp_check(y_marten, ypred_bern_marten, ppc_dens_overlay) +
  ggtitle("PPC - Marten detection"))
ggsave(plot = ppc_mart2, filename = paste0(plots_wd, "/ppc_DET_bern_marten.png"), 
       dpi = 600, width = 9, height = 5.8)
...
write.csv(Beta.results, paste0(det_results_wd, "/CTcar_DET_5seasons_beta_coeffients.csv"), row.names = TRUE)
# Default beta plot in Hmsc package
postBeta <- getPostEstimate(m, parName = "Beta")
saveRDS(postBeta, file=paste0(det_results_wd, "/postBeta_5seasons.rds"))

png(paste0(det_results_wd, "/CTcar_DET_5seasons_default_betaplot_support95.png"))
plotBeta(m, post = postBeta, param = "Support", supportLevel = 0.95)
dev.off()
  
png(paste0(det_results_wd, "/CTcar_DET_5seasons_default_betaplot_support75.png"))
plotBeta(m, post = postBeta, param = "Support", supportLevel = 0.75)
dev.off()

# my plots
# Coef plot for betas for each species
pdf(paste0(det_plots_wd, "/CTcar_DET_5seasons_beta_coef_plots.pdf"))
MCMCplot(mpost$Beta, 
           ref_ovl = TRUE,
           rank = T,
           xlab = 'ESTIMATE',
...
for (i in 1:nrow(predictors)){
    png(paste0(det_results_wd, "/Betas_covariates_coef_plot_", 
...
# write.csv(allModelFrame, paste0(det_results_wd, "/DET_all_5seasons_ModelFrame.csv"), row.names = FALSE)
```
...
class(OmegaCor)
saveRDS(OmegaCor, file=paste0(det_results_wd, "/OmegaCor_DET_5seasons.rds"))
...
png(paste0(det_results_wd , "/CTcar_DET_5seasons_default_omegaplot95.png"))
...
toPlot <- ((OmegaCor[[1]]$support > supportLevel)
           + (OmegaCor[[1]]$support < (1 - supportLevel)) > 0) * OmegaCor[[1]]$mean
png(paste0(det_results_wd, "/CTcar_DET_5seasons_default_omegaplot75.png"))
corrplot(toPlot, method = "color", 
         col = colorRampPalette(c("blue", "white", "red"))(200),
         title = paste0("random effect level: ", m$rLNames[1]), 
         mar = c(0,0,1,0))
...
write.csv(associations, paste0(det_results_wd, "/UrbanMammals_associations_DET_5seasons.csv"), row.names = FALSE)
```
...
saveRDS(VP, file = paste0(det_results_wd, "/Varpart_DET_5seasons.rds"))

VP.table <- as.data.frame(VP$vals) 
VP.table
write.csv(VP.table, paste0(det_results_wd, "/Varpart_values_DET_5seasons.csv"))


# plot var part
png(paste0(det_results_wd, "/CTcar_Carnivores_DET_5seasons_default_VP.png"), 
    width = 800)
plotVariancePartitioning(m, VP = VP, las = 2, cex.names = 0.8)
title(main = "\n \nDETECTION Urban Mammals")
dev.off()
```
...
saveRDS(predY.tree, paste0(det_results_wd, "/pred_response_5seasons_tree.rds"))
...
other attached packages:
 [1] forcats_0.5.0   stringr_1.4.0   purrr_0.3.4     readr_1.3.1     tidyr_1.1.2     tibble_3.0.3    ggplot2_3.3.2  
 [8] tidyverse_1.3.0 Hmsc_3.0-11     coda_0.19-3     dplyr_1.0.2    

loaded via a namespace (and not attached):
 [1] httr_1.4.2         maps_3.3.0         jsonlite_1.7.1     dotCall64_1.0-0    modelr_0.1.8       assertthat_0.2.1  
 [7] statmod_1.4.34     sp_1.4-2           blob_1.2.1         cellranger_1.1.0   yaml_2.2.1         pillar_1.4.6      
[13] backports_1.1.10   lattice_0.20-41    quantreg_5.61      glue_1.4.2         pROC_1.16.2        digest_0.6.25     
[19] rvest_0.3.6        colorspace_1.4-1   htmltools_0.5.0    Matrix_1.2-18      plyr_1.8.6         conquer_1.0.2     
[25] pkgconfig_2.0.3    broom_0.7.0        SparseM_1.78       haven_2.3.1        snow_0.4-3         scales_1.1.1      
[31] pracma_2.2.9       MatrixModels_0.4-1 generics_0.1.0     ellipsis_0.3.1     withr_2.3.0        BayesLogit_2.1    
[37] nnet_7.3-12        cli_2.0.2          magrittr_1.5       crayon_1.3.4       readxl_1.3.1       mcmc_0.9-7        
[43] evaluate_0.14      fansi_0.4.1        fs_1.5.0           nlme_3.1-144       MASS_7.3-51.5      xml2_1.3.2        
[49] truncnorm_1.0-8    FNN_1.1.3          tools_3.6.3        hms_0.5.3          lifecycle_0.2.0    matrixStats_0.56.0
[55] MCMCpack_1.4-9     munsell_0.5.0      reprex_0.3.0       compiler_3.6.3     tinytex_0.26       rlang_0.4.7       
[61] grid_3.6.3         rstudioapi_0.11    spam_2.5-1         rmarkdown_2.3      gtable_0.3.0       abind_1.4-5       
[67] DBI_1.1.0          R6_2.4.1           lubridate_1.7.9    knitr_1.30         utf8_1.1.4         ape_5.4-1         
[73] stringi_1.5.3      parallel_3.6.3     Rcpp_1.0.5         fields_10.3        vctrs_0.3.4        dbplyr_1.4.4      
[79] tidyselect_1.1.0   xfun_0.17         